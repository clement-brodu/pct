...
FUNCTION getRecompileLabel RETURNS CHARACTER (ipVal AS INTEGER):
  CASE ipVal:
    ...
    WHEN 7 THEN RETURN 'Not a PCT rcode'.
    OTHERWISE   RETURN '???'.
  END.
END FUNCTION.
...
PROCEDURE compileXref:
  ...
  vOutputTempDirRetry = IF cTempOutput > "" THEN vOutputTempDir + "-retry" ELSE vOutputTempDir.
  ASSIGN opError = NOT createDir(vOutputTempDir, cBase).
  IF (opError) THEN RETURN.
  ASSIGN opError = NOT createDirRecursive(vOutputTempDirRetry).
  IF (opError) THEN RETURN.
...
&ENDIF
  ...
  IF COMPILER:ERROR AND ipInFile MATCHES "*~~.cls":U AND REPLACE(vOutputTempDir, CHR(92), '/':U) MATCHES "*/~~.pctcomp/*" THEN
  DO:
    RUN pctcomp.p (IF lRelative THEN ipInFile ELSE ipInDir + '/':U + ipInFile,
